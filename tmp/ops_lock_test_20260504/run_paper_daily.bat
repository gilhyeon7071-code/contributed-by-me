...
"%PY%" p0_daily_check.py >> "%P0_STDOUT%" 2>> "%P0_STDERR%"
...
if exist "%LOCK_DIR%\run.info" (
  ...
  for /f "usebackq delims=" %%S in (`powershell -NoProfile -Command "$p='%LOCK_DIR%\run.info'; $m=%LOCK_STALE_MIN%; $pidText='%LOCK_OWNER_PID%'; if(Test-Path $p){ $age=((Get-Date)-(Get-Item $p).LastWriteTime).TotalMinutes; if($age -lt $m){ 'ACTIVE' } elseif([string]::IsNullOrWhiteSpace($pidText)){ 'STALE_NO_PID' } else { $owner=0; [void][int]::TryParse($pidText, [ref]$owner); if($owner -gt 0 -and (Get-Process -Id $owner -ErrorAction SilentlyContinue)){ 'ACTIVE_PID' } else { 'STALE_PID_DEAD' } } } else { 'UNKNOWN_NO_INFO' }"`) do set "LOCK_STATE=%%S"
) else (
...
if /I "%LOCK_STATE%"=="STALE_NO_PID" (
  echo [FAILED] stale lock without pid requires manual review: "%LOCK_DIR%"
)
if /I "%LOCK_STATE%"=="UNKNOWN_NO_INFO" (
  echo [FAILED] lock without run.info requires manual review: "%LOCK_DIR%"
)
goto :eof
