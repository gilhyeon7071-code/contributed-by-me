...
if defined PY if /I "%PY:~-4%"==".exe" set PY="%PY%"
if not defined PY (
...
for /f "usebackq delims=" %%H in (`
  powershell -NoProfile -Command "(Get-FileHash -Algorithm SHA256 -Path '%CFG%').Hash.ToLower()"
`) do set "CFG_SHA=%%H"
...
for /f "usebackq delims=" %%A in (`
  powershell -NoProfile -Command "$j = Get-Content -Raw -Encoding UTF8 '%LOCK%' | ConvertFrom-Json; [string]$j.approved_sha256"
...
echo [6.3/9] survivorship_policy_daily.py (post-candidates refresh)
%PY% survivorship_policy_daily.py
if errorlevel 1 goto :FAILED

echo [6.4/9] liquidity_filter_daily.py (post-candidates refresh)
%PY% liquidity_filter_daily.py
if errorlevel 1 goto :FAILED
...
set "KRX_KEY_FILE=E:\1_Data\_cache\krx_api_key.txt"
if "%KRX_API_KEY%"=="" (
  if exist "%KRX_KEY_FILE%" (
    set /p KRX_API_KEY=<"%KRX_KEY_FILE%"
    echo [INFO] KRX_API_KEY loaded from key file
  ) else (
    echo [INFO] KRX_API_KEY not set (sector score may run in mock mode)
  )
)
if "%KRX_API_KEY%"=="" (
...
REM ------------------------------------------------------------
REM [6.8/9] refresh p0/gate after final candidate artifacts
REM - p0_daily_check.py reads candidates_latest_meta.json, so it must run
REM   again after the candidate pipeline has finalized today's meta.
REM ------------------------------------------------------------
echo [6.8/9] p0_daily_check.py (post-candidates refresh)
%PY% p0_daily_check.py
if errorlevel 1 goto :FAILED

echo [6.9/9] gate_daily.py (post-candidates refresh)
%PY% gate_daily.py
if errorlevel 1 goto :FAILED
...
for /f "usebackq delims=" %%I in (`
  powershell -NoProfile -Command "$p='%ROOT%paper\fills.csv'; $rows=Import-Csv -Path $p -Encoding UTF8; $norm=@(); foreach($r in $rows){ try { $dt=[datetime]::ParseExact([string]$r.datetime,'yyyyMMddTHH:mm:ss',$null) } catch { continue }; $r | Add-Member -NotePropertyName ymd -NotePropertyValue ($dt.ToString('yyyyMMdd')) -Force; $norm += $r }; ($norm | ForEach-Object { $_.ymd } | Sort-Object | Select-Object -Last 1)"
`) do set "D_FILLS=%%I"
...
set "BROKER_D=%RUN_TS:~0,4%%RUN_TS:~5,2%%RUN_TS:~8,2%"
...
if /I "%BROKER_MODE%"=="OFF" (
  ...
  if /I "%BROKER_MODE%"=="DRY" (
    %PY% tools\kis_order_dispatch_from_exec.py --date %BROKER_D% --mock %BROKER_MOCK% %BROKER_VALID_ARGS%
    if errorlevel 1 goto :FAILED
    ...
    if /I "%BROKER_MODE%"=="APPLY" (
      %PY% tools\kis_order_dispatch_from_exec.py --date %BROKER_D% --mock %BROKER_MOCK% %BROKER_VALID_ARGS% --apply
      if errorlevel 1 goto :FAILED
      if "%BROKER_CANCEL_OPEN%"=="1" (
...
if not defined RUN_TS set "RUN_TS=%DATE%_%TIME%"
>> "%LAST_LOG%" echo [FAILED] ts=%RUN_TS% ERRORLEVEL=%ERRORLEVEL%
popd
endlocal & exit /b 1
