...
REM [16.922/16] refresh SSOT drift monitor snapshots (report-only)
echo [16.922/16] tools\build_drift_monitor.py --date %D%
"%PY%" tools\build_drift_monitor.py --date %D%
if errorlevel 1 (
  call :POST_CHAIN_STEP_FAIL "[16.922/16] build_drift_monitor.py"
)

REM [16.924/16] repair live_fills rows missing from RootB ledger before monitor
echo [16.924/16] tools\repair_rootb_ledger_missing_live_fills.py --apply
"%PY%" tools\repair_rootb_ledger_missing_live_fills.py --start-ymd %PAPER_OPER_START_YMD% --apply
if errorlevel 1 (
  call :POST_CHAIN_STEP_FAIL "[16.924/16] repair_rootb_ledger_missing_live_fills.py --apply"
)
...
if errorlevel 1 (
  echo [WARN] ledger_live_fills_dry_run_report found missing rows - report only
)

REM [16.93/16] recovery SSOT chain report after reconciliation and post-chain artifacts
echo [16.93/16] tools\build_recovery_ssot_chain_report.py
"%PY%" tools\build_recovery_ssot_chain_report.py
if errorlevel 1 (
  echo [WARN] build_recovery_ssot_chain_report found chain issues - report only
)
...
"%PY%" tools\orderflow_hawkes_glr.py
if errorlevel 1 (
  call :POST_CHAIN_STEP_FAIL "[16.957a2/16] orderflow_hawkes_glr.py"
)
