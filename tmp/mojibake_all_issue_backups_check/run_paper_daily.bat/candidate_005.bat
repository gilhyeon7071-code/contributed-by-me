...
for /f "usebackq delims=" %%H in (`
  %PY% "%ROOT%batch_utils.py" sha256 "%CFG%"
`) do set "CFG_SHA=%%H"
...
for /f "usebackq delims=" %%A in (`
  %PY% "%ROOT%batch_utils.py" json-get "%LOCK%" approved_sha256
...
for /f "usebackq delims=" %%I in (`
  %PY% "%ROOT%batch_utils.py" csv-max-date "%ROOT%paper\fills.csv" datetime
`) do set "D_FILLS=%%I"
