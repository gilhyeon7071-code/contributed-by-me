...
call :TRY_PY_EXE "%ROOT%.venv\Scripts\python.exe"
call :TRY_PY_EXE "E:\vibe\buffett\.venv\Scripts\python.exe"
call :TRY_PY_EXE "C:\Users\jjtop\AppData\Local\Programs\Python\Python312\python.exe"
if not defined PY call :TRY_PY_CMD python
if not defined PY call :TRY_PY_CMD "py -3"
if defined PY (
...
REM ------------------------------------------------------------
if "%CANDIDATES_REFRESHED%"=="1" (
  if "%POST_REFRESH_NEEDED%"=="1" (
    call :LOG_MSG "[6.26/9] candidates changed and refresh needed -> run post-candidates refresh trio"
    ...
    call :LOG_MSG "[6.26/9] candidates changed but prices already current -> skip post-candidates refresh trio"
  )
) else (
  call :LOG_MSG "[6.26/9] candidates unchanged -> skip post-candidates refresh trio"
)
...
set "D_FILLS="
call "%ROOT%utils\set_csv_max_date.bat" "%ROOT%paper\fills.csv" datetime D_FILLS
...
if exist "E:\vibe\buffett\tools\build_dashboard_state.py" (
  echo [16.8b/16] E:\vibe\buffett\tools\build_dashboard_state.py
  %PY% E:\vibe\buffett\tools\build_dashboard_state.py
  if errorlevel 1 goto :FAILED
)
...
if not "%PAPER_FIX_RC%"=="0" goto :FAILED
