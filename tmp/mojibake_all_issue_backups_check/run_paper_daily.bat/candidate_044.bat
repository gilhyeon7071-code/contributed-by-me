...
if not defined RUN_DAILY_STDERR_CAPTURED (
  ...
  if exist "%ROOT%2_Logs\run_paper_daily.lock" (
    if exist "%ROOT%2_Logs\run_paper_daily_last.txt" >> "%ROOT%2_Logs\run_paper_daily_last.txt" echo [WRAPPER_CLEANUP] orphan lock detected after rc=!WRAP_RC!
    del /f /q "%ROOT%2_Logs\run_paper_daily.lock\run.info" >nul 2>nul
    rmdir "%ROOT%2_Logs\run_paper_daily.lock" >nul 2>nul
  )
  if exist "%ROOT%2_Logs\run_paper_daily_last.txt" >> "%ROOT%2_Logs\run_paper_daily_last.txt" echo [WRAPPER_EXIT] rc=!WRAP_RC!
  if not "!WRAP_RC!"=="0" (
    for %%R in (!WRAP_RC!) do endlocal & exit /b %%R
  )
  endlocal & exit /b 0
)
pushd "%ROOT%" || (
...
if not defined PY (
  ...
  echo                  "%ROOT%_runtime\python312-embed\python.exe"
...
"%PY%" krx_update_clean_incremental.py --base . --min-uni %KRX_MIN_UNI% --probe-cap 700
if errorlevel 1 goto :FAILED
...
call :LOG_STEP_BEGIN "[6.965/9] tools\\news_score_daily.py"
"%PY%" tools\news_score_daily.py
set "RC=%ERRORLEVEL%"
call :LOG_STEP_END "[6.965/9]" %RC%
if not "%RC%"=="0" goto :FAILED
...
REM ???? [6.24b/9] Forward Estimate Snapshot (ÃßÁ¤PER / TTM / PEG) ????????????????????????????
...
"%PY%" tools\run_future_signal_daily.py
set "RC=%ERRORLEVEL%"
...
REM ------------------------------------------------------------
if "%PAPER_EXEC_DATE_MODE%"=="" set "PAPER_EXEC_DATE_MODE=HYBRID"
...
"%PY%" -c "import glob,json,os,sys; fs=glob.glob(r'%ROOT%2_Logs\paper_pending_report_*.json'); f=max(fs,key=os.path.getmtime); j=json.load(open(f,'r',encoding='utf-8')); pend=j.get('pending',[]) or []; act=j.get('active',[]) or []; px=str(j.get('prices_date_max') or ''); stf=r'%ROOT%2_Logs\pending_entry_status_latest.json'; st=(json.load(open(stf,'r',encoding='utf-8-sig')) if os.path.exists(stf) else {}); zero_reason=str(st.get('max_new_zero_reason') or '').strip().lower(); max_new=int(st.get('max_new') or 0); entry_ready=int(st.get('entry_ready') or 0); blocked=(max_new<=0 and entry_ready<=0); ok_same=(len(pend)==0) or all((str(r.get('entry_date') or '')==px and str(r.get('reason') or '') in {'SAME_DAY_ENTRY_WAIT_NEXT_SESSION','NO_PRICES_AFTER_ENTRY_DATE'}) for r in pend); ok_blocked=bool(blocked and len(pend)>0 and all((str(r.get('reason') or '')=='NO_PRICES_AFTER_ENTRY_DATE' and str(r.get('last_price_date_for_code') or '')==str(r.get('entry_date') or '')) for r in pend)); ok=(ok_same or ok_blocked); print('LATEST',f,'prices_date_max',px,'pending',len(pend),'active',len(act),'ok_same_day_pending',ok_same,'blocked',blocked,'zero_reason',zero_reason,'max_new',max_new,'entry_ready',entry_ready,'ok_blocked_pending',ok_blocked,'ok',ok); sys.exit(0 if ok else 3)"
if errorlevel 1 (
