...
if exist "%ROOT%paper\fills.csv" (
  for /f %%I in ('"%PY%" -c "import pandas as pd; p=r'%ROOT%paper\fills.csv'; df=pd.read_csv(p,dtype=str,encoding='utf-8-sig'); dt=df.get('datetime',pd.Series(dtype=str)).astype(str).str.replace(r'\\D','',regex=True).str[:8]; sd=df.get('side',pd.Series(dtype=str)).astype(str).str.upper().str.strip(); m=(sd.eq('BUY')*dt.str.len().eq(8)); b=dt[m.astype(bool)]; a=dt[dt.str.len().eq(8)]; print(b.max() if len(b) else (a.max() if len(a) else ''))"') do set "D_FILLS=%%I"
)
...
"%PY%" -c "import pandas as pd,sys; D=r'%D%'; op=rf'%ROOT%paper\orders_{D}_exec.xlsx'; fp=r'%ROOT%paper\fills.csv'; odf=pd.read_excel(op,dtype=str); uo=sorted(odf['exec_date'].astype(str).str.strip().unique().tolist()); fdf=pd.read_csv(fp,dtype=str,encoding='utf-8-sig'); dt=fdf.get('datetime',pd.Series(dtype=str)).astype(str).str.replace(r'\\D','',regex=True).str[:8]; sd=fdf.get('side',pd.Series(dtype=str)).astype(str).str.upper().str.strip(); m=(sd.eq('BUY')*dt.str.len().eq(8)); b=dt[m.astype(bool)]; a=dt[dt.str.len().eq(8)]; Df=(b.max() if len(b) else (a.max() if len(a) else '')); print(f'[CHAIN_D] D={D} orders_exec_date_unique={uo} fills_D_by_rule={Df}'); sys.exit(0 if (uo==[D] and Df==D) else 2)"
if errorlevel 1 goto :FAILED
