...
set "CFG_SHA="
set "CFG_SHA_TMP=%ROOT%2_Logs\_cfg_sha.tmp"
if exist "%CFG_SHA_TMP%" del /q "%CFG_SHA_TMP%" >nul 2>nul
"%PY_RAW%" "%ROOT%batch_utils.py" sha256 "%CFG%" > "%CFG_SHA_TMP%" 2>nul
if exist "%CFG_SHA_TMP%" set /p CFG_SHA=<"%CFG_SHA_TMP%"
if exist "%CFG_SHA_TMP%" del /q "%CFG_SHA_TMP%" >nul 2>nul
...
set "APPROVED_SHA="
set "APPROVED_SHA_TMP=%ROOT%2_Logs\_approved_sha.tmp"
if exist "%APPROVED_SHA_TMP%" del /q "%APPROVED_SHA_TMP%" >nul 2>nul
"%PY_RAW%" "%ROOT%batch_utils.py" json-get "%LOCK%" approved_sha256 > "%APPROVED_SHA_TMP%" 2>nul
if exist "%APPROVED_SHA_TMP%" set /p APPROVED_SHA=<"%APPROVED_SHA_TMP%"
if exist "%APPROVED_SHA_TMP%" del /q "%APPROVED_SHA_TMP%" >nul 2>nul
...
if exist "%D_FILLS_TMP%" del /q "%D_FILLS_TMP%" >nul 2>nul
"%PY_RAW%" "%ROOT%batch_utils.py" csv-max-date "%ROOT%paper\fills.csv" datetime > "%D_FILLS_TMP%" 2>nul
