...
set "CFG_SHA="
for /f "delims=" %%H in ('"%PY_RAW%" "%ROOT%batch_utils.py" sha256 "%CFG%"') do set "CFG_SHA=%%H"
...
set "APPROVED_SHA="
for /f "delims=" %%A in ('"%PY_RAW%" "%ROOT%batch_utils.py" json-get "%LOCK%" approved_sha256') do set "APPROVED_SHA=%%A"
...
set "D_FILLS="
for /f "delims=" %%I in ('"%PY_RAW%" "%ROOT%batch_utils.py" csv-max-date "%ROOT%paper\fills.csv" datetime') do set "D_FILLS=%%I"
