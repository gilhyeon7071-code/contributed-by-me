...
OUT_JSON = LOG_DIR / "bear_signal_direction_scan_latest.json"
OUT_SUMMARY = LOG_DIR / "bear_signal_direction_scan_summary_latest.csv"
OUT_ROWS = LOG_DIR / "bear_signal_direction_scan_rows_latest.csv"
OUT_MD = LOG_DIR / "bear_signal_direction_scan_latest.md"

REGIME = "BEAR"


def load_common() -> Any:
    spec = importlib.util.spec_from_file_location("scan_signal_directions_common_for_bear", COMMON_SCAN)
...
def main() -> int:
    ...
    if len(current_positive):
        conclusion = "CURRENT_BEAR_POSITIVE_SINGLE_SIGNAL_BRANCHES_FOUND_READ_ONLY"
    elif len(historical_positive):
        conclusion = "HISTORICAL_BEAR_POSITIVE_BRANCHES_FOUND_CURRENT_NOT_CONFIRMED"
    else:
        conclusion = "NO_BEAR_SINGLE_SIGNAL_BRANCH_SUPPORTED"
    ...
    payload = {
        ...
        "scope": "bear_signal_direction_scan_read_only",
        "classification": "BEAR_SIGNAL_DIRECTION_SCAN_READ_ONLY",
        ...
        "method": {
            "population": "market_regime == BEAR only",
        ...
        "row_counts": {
            ...
            "bear_rows": int(len(base)),
            "current_bear_rows": int(len(base[base["date"].ge(common.CURRENT_START)])),
    ...
    for row in payload["top_positive_branches"][:10]:
        md.append(
            f"- {row['sample']} / BEAR / {row['signal']} {row['direction']} / {row['horizon']}: "
