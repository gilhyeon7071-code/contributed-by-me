...
OUT_JSON = LOG_DIR / "transition_signal_direction_scan_latest.json"
OUT_SUMMARY = LOG_DIR / "transition_signal_direction_scan_summary_latest.csv"
OUT_ROWS = LOG_DIR / "transition_signal_direction_scan_rows_latest.csv"
OUT_MD = LOG_DIR / "transition_signal_direction_scan_latest.md"

REGIME = "TRANSITION"


def load_common() -> Any:
    spec = importlib.util.spec_from_file_location("scan_signal_directions_common_for_transition", COMMON_SCAN)
...
def main() -> int:
    ...
    if len(current_positive):
        conclusion = "CURRENT_TRANSITION_POSITIVE_SINGLE_SIGNAL_BRANCHES_FOUND_READ_ONLY"
    elif len(historical_positive):
        conclusion = "HISTORICAL_TRANSITION_POSITIVE_BRANCHES_FOUND_CURRENT_NOT_CONFIRMED"
    else:
        conclusion = "NO_TRANSITION_SINGLE_SIGNAL_BRANCH_SUPPORTED"
    ...
    payload = {
        ...
        "scope": "transition_signal_direction_scan_read_only",
        "classification": "TRANSITION_SIGNAL_DIRECTION_SCAN_READ_ONLY",
        ...
        "method": {
            "population": "market_regime == TRANSITION only",
        ...
        "row_counts": {
            ...
            "transition_rows": int(len(base)),
            "current_transition_rows": int(len(base[base["date"].ge(common.CURRENT_START)])),
    ...
    md = [
        "# TRANSITION Signal Direction Scan",
        "",
    ...
    for row in payload["top_positive_branches"][:10]:
        md.append(
            f"- {row['sample']} / TRANSITION / {row['signal']} {row['direction']} / {row['horizon']}: "
