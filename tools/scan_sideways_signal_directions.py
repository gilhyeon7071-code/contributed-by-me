...
OUT_JSON = LOG_DIR / "sideways_signal_direction_scan_latest.json"
OUT_SUMMARY = LOG_DIR / "sideways_signal_direction_scan_summary_latest.csv"
OUT_ROWS = LOG_DIR / "sideways_signal_direction_scan_rows_latest.csv"
OUT_MD = LOG_DIR / "sideways_signal_direction_scan_latest.md"
...
def load_report_module() -> Any:
    spec = importlib.util.spec_from_file_location("report_backtest_v41_1_for_sideways_signal_scan", REPORT_BACKTEST)
...
def main() -> int:
    ...
    base = factors[factors["market_regime"].astype(str).eq("SIDEWAYS")].copy()
    ...
    for sample in samples:
        ...
        for sig in SIGNALS:
            ...
            if sig["direction"] != "true" and f"{sig['field']}_rank_pct" not in sample_df.columns:
                continue
            picked = sample_df[signal_mask(sample_df, sig)].copy()
            for h in HORIZONS:
                summary_rows.append(summarize(picked, sample, "SIDEWAYS", sig, h))
    ...
    if len(current_positive):
        conclusion = "CURRENT_SIDEWAYS_POSITIVE_SINGLE_SIGNAL_BRANCHES_FOUND_READ_ONLY"
    elif len(historical_positive):
        conclusion = "HISTORICAL_SIDEWAYS_POSITIVE_BRANCHES_FOUND_CURRENT_NOT_CONFIRMED"
    else:
        conclusion = "NO_SIDEWAYS_SINGLE_SIGNAL_BRANCH_SUPPORTED"
    ...
    payload = {
        ...
        "scope": "sideways_signal_direction_scan_read_only",
        "classification": "SIDEWAYS_SIGNAL_DIRECTION_SCAN_READ_ONLY",
        ...
        "method": {
            "population": "market_regime == SIDEWAYS only",
        ...
        "row_counts": {
            ...
            "sideways_rows": int(len(base)),
            "current_sideways_rows": int(len(base[base["date"].ge(CURRENT_START)])),
    ...
    md = [
        "# SIDEWAYS Signal Direction Scan",
        "",
    ...
    for row in payload["top_positive_branches"][:10]:
        md.append(
            f"- {row['sample']} / SIDEWAYS / {row['signal']} {row['direction']} / {row['horizon']}: "
