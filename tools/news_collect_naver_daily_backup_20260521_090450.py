...
def _extract_naver_finance_links(html_text: str, base_url: str, source: str, reference_ymd: str) -> List[Dict[str, object]]:
    link_re = re.compile(r"<a\b[^>]*href=[\"'](?P<href>[^\"']+)[\"'][^>]*>(?P<title>.*?)</a>", re.IGNORECASE | re.DOTALL)
    rows: List[Dict[str, object]] = []
    seen: set[str] = set()
    source_l = str(source or "").upper()
    for m in link_re.finditer(str(html_text or "")):
        href = html.unescape(str(m.group("href") or "")).strip()
        title = _strip_html(html.unescape(str(m.group("title") or ""))).strip()
        ...
        if source_l == "NAVER_FINANCE_NEWS" and ("/news/" not in path or not leaf.endswith("_read.naver")):
            continue
        if source_l == "NAVER_FINANCE_RESEARCH" and ("/research/" not in path or not leaf.endswith("_read.naver")):
            continue
...
def _extract_entity_tags(text: str) -> List[str]:
    ...
    if not raw:
        return []
    t = raw.upper()
    clean_rules: List[Tuple[str, Tuple[str, ...]]] = [
        ("금리", ("금리", "기준금리", "국채", "채권금리", "FOMC", "FED", "BOND YIELD")),
        ("환율", ("환율", "원달러", "USD/KRW", "DOLLAR", "달러", "YEN", "외환")),
    ...
    clean_tags: List[str] = []
    for label, kws in clean_rules:
        if any(kw.upper() in t for kw in kws):
            clean_tags.append(label)
    if clean_tags:
        return clean_tags[:6]
    rules: List[Tuple[str, Tuple[str, ...]]] = [
        ("금리", ("금리", "기준금리", "국채", "채권금리", "FOMC", "FED", "BOND YIELD")),
        ("환율", ("환율", "원달러", "USD/KRW", "DOLLAR", "달러", "YEN", "엔화")),
        ("인플레이션", ("물가", "인플레이션", "CPI", "PPI")),
        ("반도체", ("반도체", "SEMICON", "메모리", "HBM")),
        ("배터리", ("2차전지", "배터리", "BATTERY", "양극재", "음극재", "전해질")),
        ("조선", ("조선", "SHIP", "LNG", "해운")),
...
def _cleanup_old_articles(con: sqlite3.Connection, keep_days: int = 7) -> int:
    """keep_days 이전 기사 삭제. 기본 7일 보관 (max_age_hours=72h + lookback_days=3 + 여유 1일)."""
        ...
        try:
            ...
            for attempt in range(1, attempts + 1):
                ...
                try:
                    ...
                    keep_days = max(1, int(str(os.getenv("NEWS_DB_KEEP_DAYS", "7")).strip() or "7"))
