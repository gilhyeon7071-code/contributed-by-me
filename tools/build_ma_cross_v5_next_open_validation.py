"""Research-only V5 next-open validation for the unranked MA20/MA60 cross."""
...
CONTRACT_PATH = LOG_DIR / "strategy_execution_contract_v5_ma_cross_next_open.json"
PREFIX = LOG_DIR / "ma_cross_v5_next_open"
...
def _returns(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for horizon, sessions in HORIZONS:
        ...
        out[f"v5_return_{horizon}"] = exit_close.div(entry_open).sub(1).where(valid)
    return out


def _summary(chosen: pd.DataFrame, horizon: str) -> list[dict[str, object]]:
    result = []
    for split, group in chosen.groupby("v5_split", sort=True):
        result.append({"strategy": "MA_CROSS_UP", "horizon": horizon, "selection_variant": "ALL_SIGNAL_ROWS", "split": split, "trades": int(len(group)), "avg_gross": float(group.gross_return.mean()), "avg_net": float(group.net_return.mean()), "median_net": float(group.net_return.median()), "win_rate": float(group.net_return.gt(0).mean()), "avg_same_day_baseline_gross": float(group.same_day_baseline_gross.mean()), "avg_same_day_baseline_net": float(group.same_day_baseline_net.mean()), "avg_gross_excess_vs_same_day": float(group.gross_excess_vs_same_day.mean()), "avg_net_excess_vs_same_day": float(group.net_excess_vs_same_day.mean())})
    return result


def _oos(chosen: pd.DataFrame, horizon: str) -> dict[str, object]:
    oos = chosen.loc[chosen.v5_split.eq("OOS_202501_202505")].sort_values("net_return", ascending=False)
    trimmed = oos.iloc[5:]
    return {"strategy": "MA_CROSS_UP", "horizon": horizon, "selection_variant": "ALL_SIGNAL_ROWS", "trades": int(len(oos)), "avg_net": float(oos.net_return.mean()) if len(oos) else None, "median_net": float(oos.net_return.median()) if len(oos) else None, "win_rate": float(oos.net_return.gt(0).mean()) if len(oos) else None, "top5_net_sum": float(oos.head(5).net_return.sum()) if len(oos) else None, "avg_net_excluding_top5": float(trimmed.net_return.mean()) if len(trimmed) else None}


def main() -> int:
    contract = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
    lib = _library(); base = lib._base(); report = base._load_report_module()
    ...
    factors["v5_split"] = pd.NA
    for label, (start, end) in SPLITS.items(): factors.loc[factors.date.between(start, end), "v5_split"] = label
    pre = factors.loc[factors.v5_split.notna()].copy(); precoverage = []
    for split, group in pre.groupby("v5_split", sort=True):
    ...
    panel = factors.loc[factors.v5_split.notna() & factors.market_effective.isin(["KOSPI", "KOSDAQ"]) & pd.to_numeric(factors.close, errors="coerce").gt(0) & pd.to_numeric(factors.open, errors="coerce").gt(0) & pd.to_numeric(factors.value, errors="coerce").gt(0)].copy()
    signal = panel.MA_CROSS_UP.fillna(False)
    coverage = []
    for split, group in panel.groupby("v5_split", sort=True): coverage.append({"split": split, "panel_rows": int(len(group)), "unique_dates": int(group.date.nunique()), "unique_codes": int(group.code.nunique()), "ma_cross_signals": int(signal.loc[group.index].sum()), "valid_next_open_h1": int(group.v5_return_h1.notna().sum()), "valid_next_open_h2": int(group.v5_return_h2.notna().sum()), "valid_next_open_h5": int(group.v5_return_h5.notna().sum())})
    summary_rows, diagnostic_rows, count_rows, selected_frames = [], [], [], []
    for horizon, _ in HORIZONS:
        ret_col = f"v5_return_{horizon}"; valid = pd.to_numeric(panel[ret_col], errors="coerce").notna(); chosen = panel.loc[signal & valid].copy()
        baseline = panel.loc[valid].groupby("date", sort=False)[ret_col].mean()
        chosen["gross_return"] = chosen[ret_col]; chosen["net_return"] = _net(chosen.gross_return)
        chosen["same_day_baseline_gross"] = chosen.date.map(baseline); chosen["same_day_baseline_net"] = _net(chosen.same_day_baseline_gross)
        chosen["gross_excess_vs_same_day"] = chosen.gross_return - chosen.same_day_baseline_gross; chosen["net_excess_vs_same_day"] = chosen.net_return - chosen.same_day_baseline_net
        chosen["horizon"], chosen["selection_variant"] = horizon, "ALL_SIGNAL_ROWS"
        summary_rows.extend(_summary(chosen, horizon)); diagnostic_rows.append(_oos(chosen, horizon)); selected_frames.append(chosen)
        count_rows.append({"horizon": horizon, "selection_variant": "ALL_SIGNAL_ROWS", "selected_trades": int(len(chosen)), "unique_dates": int(chosen.date.nunique()), "max_trades_per_date": int(chosen.groupby("date", sort=False).size().max()) if len(chosen) else 0})
    ...
    print(json.dumps({"status": "OK", "variants": len(HORIZONS), "selected_trades": int(sum(row["selected_trades"] for row in count_rows))}, ensure_ascii=False))
    return 0
