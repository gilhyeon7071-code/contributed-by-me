"""Read-only Stage 3 execution validation for BREAKOUT_252D h1 TOP_5."""
...
PREFIX = LOG_DIR / "breakout_252d_h1_top5_execution"
...
def main() -> int:
    ...
    valid_return = pd.to_numeric(panel["path_return_h1"], errors="coerce").notna()
    selected = signal & rank.reindex(panel.index).le(5).fillna(False) & valid_return

    baseline = panel.loc[valid_return].groupby("date", sort=False)["path_return_h1"].mean().rename("same_day_baseline_gross")
    all_signals = panel.loc[signal & valid_return].copy()
    chosen = panel.loc[selected].copy()
    for frame in (all_signals, chosen):
        frame["gross_return"] = frame["path_return_h1"]
    ...
    payload = {
        ...
        "entry": "same_close",
        "exit": "exact_global_trading_session_close_h1",
        "cost": {"fee_pct": FEE, "slippage_pct": SLIPPAGE},
        "baseline": "same_date_full_universe_mean_h1_with_equal_costs",
