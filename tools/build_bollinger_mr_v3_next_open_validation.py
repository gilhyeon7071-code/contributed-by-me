"""Research-only V3 next-open execution validation for a fixed Bollinger MR grid."""
...
CONTRACT_PATH = LOG_DIR / "strategy_execution_contract_v3_bollinger_mr_next_open.json"
PREFIX = LOG_DIR / "bollinger_mr_v3_next_open"
...
def _next_open_returns(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for horizon, sessions in (("h1", 1), ("h2", 2), ("h5", 5)):
        ...
        out[f"v3_return_{horizon}"] = exit_close.div(entry_open).sub(1.0).where(valid)
    return out


def _summary(frame: pd.DataFrame, horizon: str, n: int) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for split, group in frame.groupby("v3_split", sort=True):
        rows.append({
            "strategy": "BOLLINGER_MR", "horizon": horizon, "selection_variant": f"TOP_{n}",
            "split": split, "trades": int(len(group)),
            "avg_gross": float(group["gross_return"].mean()), "avg_net": float(group["net_return"].mean()),
            "median_net": float(group["net_return"].median()), "win_rate": float(group["net_return"].gt(0).mean()),
...
def _oos_diagnostic(frame: pd.DataFrame, horizon: str, n: int) -> dict[str, object]:
    oos = frame.loc[frame["v3_split"].eq("OOS_202501_202505")].sort_values("net_return", ascending=False)
    trimmed = oos.iloc[5:]
    return {
        "strategy": "BOLLINGER_MR", "horizon": horizon, "selection_variant": f"TOP_{n}",
        "trades": int(len(oos)), "avg_net": float(oos["net_return"].mean()) if len(oos) else None,
...
def main() -> int:
    ...
    mapping_meta = {
        "reference_start": str(LABEL_START.date()), "uniquely_mapped_codes": int(len(label_map)),
    ...
    factors["v3_split"] = pd.NA
    for label, (start, end) in SPLITS.items():
        factors.loc[factors["date"].between(start, end), "v3_split"] = label
    pre_panel = factors.loc[factors["v3_split"].notna()].copy()
    precoverage_rows: list[dict[str, object]] = []
    for split, group in pre_panel.groupby("v3_split", sort=True):
        ...
        precoverage_rows.append({"split": split, "factor_rows": int(len(group)), "unique_dates": int(group["date"].nunique()), "unique_codes": int(group["code"].nunique()), "market_rows": int(market.sum()), "positive_close_rows": int(close_ok.sum()), "positive_open_rows": int(open_ok.sum()), "positive_value_rows": int(value_ok.sum()), "all_panel_filter_rows": int((market & close_ok & open_ok & value_ok).sum())})
    panel = factors[factors["v3_split"].notna() & factors["market_effective"].isin(["KOSPI", "KOSDAQ"]) & pd.to_numeric(factors["close"], errors="coerce").gt(0) & pd.to_numeric(factors["open"], errors="coerce").gt(0) & pd.to_numeric(factors["value"], errors="coerce").gt(0)].copy()
    signal = panel["MR_BOLLINGER"].fillna(False)
    rank = panel.loc[signal, "strategy_z20"].groupby(panel.loc[signal, "date"], sort=False).rank(method="first", ascending=True)
    coverage_rows: list[dict[str, object]] = []
    for split, group in panel.groupby("v3_split", sort=True):
        coverage_rows.append({"split": split, "panel_rows": int(len(group)), "unique_dates": int(group["date"].nunique()), "unique_codes": int(group["code"].nunique()), "bollinger_signals": int(signal.loc[group.index].sum()), "valid_next_open_h1": int(group["v3_return_h1"].notna().sum()), "valid_next_open_h2": int(group["v3_return_h2"].notna().sum()), "valid_next_open_h5": int(group["v3_return_h5"].notna().sum())})
    summary_rows: list[dict[str, object]] = []
    diagnostic_rows: list[dict[str, object]] = []
    selected_frames: list[pd.DataFrame] = []
    count_rows: list[dict[str, object]] = []
    for horizon, n in GRID:
        return_column = f"v3_return_{horizon}"
    ...
    payload = {"generated_at": datetime.now().isoformat(timespec="seconds"), "contract_version": contract["contract_version"], "splits": {label: [str(start.date()), str(end.date())] for label, (start, end) in SPLITS.items()}, "counts": count_rows, "coverage": coverage_rows, "prepanel_coverage": precoverage_rows, "market_mapping": mapping_meta, "price_history_integrity": integrity, "operational_change": False, "broker_order": False}
