...
LOG_DIR = ROOT / "2_Logs"
SUBSCRIPTION_STATE_PATH = LOG_DIR / "kis_ws_index_subscriptions_latest.json"
MARKET_EVENT_LATEST_PATH = LOG_DIR / "kis_ws_index_latest.json"
logger = logging.getLogger("kis_ws_index_realtime")
...
def _parse_codes(raw: str) -> List[str]:
    ...
    for x in str(raw or "").split(","):
        ...
        if not t:
            continue
        # Index codes are usually 4 digits in KIS Open API (e.g., 0001)
        vals.append(t)
...
def main() -> int:
    ...
    ap.add_argument("--codes", default="0001,1001,2001", help="Comma-separated index symbols")
    ap.add_argument("--channels", default="trade", help="trade or raw tr_id list")
    ...
    if worker_suffix:
        SUBSCRIPTION_STATE_PATH = LOG_DIR / f"kis_ws_index_subscriptions_latest{worker_suffix}.json"
        # Intentionally removed MARKET_EVENT_LATEST_PATH suffix so detector can always find kis_ws_index_latest.json
    ...
    requested_subscription_count = int(len(tr_ids) * len(codes))
