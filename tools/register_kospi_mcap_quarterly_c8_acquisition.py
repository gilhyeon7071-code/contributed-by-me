...
def main() -> int:
    parser = argparse.ArgumentParser(description="Capture immutable C8 official-source acquisition evidence.")
    parser.add_argument("--request", type=Path, default=None)
    parser.add_argument(
        "--expect-verdict",
        choices=["C8_ACQUISITION_CAPTURE_READY", "FAIL_C8_ACQUISITION_CAPTURE"],
        default=None,
    )
    parser.add_argument("--no-write-report", action="store_true")
    args = parser.parse_args()

    report = capture_acquisition_evidence(request_path=args.request)
    outputs: list[str] = []
    if not args.no_write_report:
        versioned, latest = write_acquisition_report(report)
        outputs = [str(versioned), str(latest)]
    print(
        json.dumps(
            {
                ...
                "evidence_published": report["evidence_published"],
