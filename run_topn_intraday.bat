...
REM [2026-09-04] --require-market-hours 추가. 저녁 배치(22:10)는 이 플래그를 주지 않는다.
REM   모든 작업이 StartWhenAvailable=True 라 정전 후 로그인하면 놓친 실행이 즉시 따라온다.
REM   장중 배치가 예컨대 월 08:00 에 따라오면 거래일 가드는 통과하고(월요일이니까)
REM   디스패처 세션 가드는 mock 이라 면제된다 -> 장 시작 전 발주가 가능했다.
REM   창은 TOPN_MARKET_HOURS(기본 0900-1530). 수동 실행은 TOPN_GUARD_IGNORE_HOURS=1.
"%ROOT%_runtime\python312-embed\python.exe" "%ROOT%tools\topn_session_guard.py" --require-market-hours >> "%LOG%" 2>&1
set "GRC=%ERRORLEVEL%"
...
REM [2026-09-12] Emit the cycle-status artifact right AFTER the [END] marker,
REM   so it describes the cycle that just finished. Read-only on the log.
REM   Never fail the cycle over a display artifact. PLANS (207) item 1 / (378).
"%ROOT%_runtime\python312-embed\python.exe" "%ROOT%tools\topn_cycle_status.py" >nul 2>&1
endlocal & exit /b 0
