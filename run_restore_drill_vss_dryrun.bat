...
if not defined PY_EXE (
  echo [RESTORE_DRILL_VSS_DRYRUN] FAILED python runtime not found
  exit /b 9009
)

echo [RESTORE_DRILL_VSS_DRYRUN] PY=%PY_EXE%
"%PY_EXE%" "%ROOT%tools\restore_drill_vss_scripts.py" %*
set "RC=%ERRORLEVEL%"
echo [RESTORE_DRILL_VSS_DRYRUN] exit=%RC%
exit /b %RC%
