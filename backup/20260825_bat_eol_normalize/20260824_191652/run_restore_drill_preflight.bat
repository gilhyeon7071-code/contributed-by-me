...
if not defined PY_EXE (
  echo [RESTORE_DRILL_PREFLIGHT] FAILED python runtime not found
  exit /b 9009
)

echo [RESTORE_DRILL_PREFLIGHT] PY=%PY_EXE%
"%PY_EXE%" "%ROOT%tools\restore_drill_preflight.py" %*
set "RC=%ERRORLEVEL%"
echo [RESTORE_DRILL_PREFLIGHT] exit=%RC%
exit /b %RC%
