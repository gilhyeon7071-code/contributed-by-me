...
set "VERIFY_RISK_FILTER_TESTS=%VERIFY_RISK_FILTER_TESTS%"
if "%VERIFY_RISK_FILTER_TESTS%"=="" set "VERIFY_RISK_FILTER_TESTS=1"
set "RISK_FILTER_TEST=%ROOT%\tests\test_risk_filter_regressions.py"
set "VERIFY_LOGIC_JUDGMENT_TESTS=%VERIFY_LOGIC_JUDGMENT_TESTS%"
if "%VERIFY_LOGIC_JUDGMENT_TESTS%"=="" set "VERIFY_LOGIC_JUDGMENT_TESTS=1"
set "LOGIC_JUDGMENT_TEST=%ROOT%\tests\test_logic_judgment_module_progress.py"
...
if "%VERIFY_LOGIC_JUDGMENT_TESTS%"=="1" (
  call :run_logic_judgment_tests
  if errorlevel 1 (
    set "RC=%ERRORLEVEL%"
    goto :end
  )
...
if "%VERIFY_RISK_FILTER_TESTS%"=="1" (
  call :run_risk_filter_tests
  if errorlevel 1 (
    set "RC=%ERRORLEVEL%"
    goto :end
  )
...
:open_html
if "%VERIFY_OPEN_TRVAL%"=="" set "VERIFY_OPEN_TRVAL=0"
set "TRVAL_HTML=%ROOT%\2_Logs\trading_stage_validation_latest.html"
...
:open_html_found
if "%VERIFY_OPEN_REPORT%"=="" set "VERIFY_OPEN_REPORT=0"
if "!VERIFY_OPEN_REPORT!"=="1" (
...
:run_logic_judgment_tests
if not exist "%LOGIC_JUDGMENT_TEST%" (
  echo [ERROR] logic judgment regression test missing: %LOGIC_JUDGMENT_TEST%
  exit /b 44
)
echo [INFO] Running logic judgment regression tests...
%PY% -m unittest "%LOGIC_JUDGMENT_TEST%"
if errorlevel 1 (
  echo [ERROR] logic judgment regression tests failed.
  exit /b 44
)
echo [INFO] Logic judgment regression tests passed.
exit /b 0

:run_risk_filter_tests
if not exist "%RISK_FILTER_TEST%" (
  echo [ERROR] risk filter regression test missing: %RISK_FILTER_TEST%
  exit /b 43
)
echo [INFO] Running risk filter regression tests...
%PY% -m unittest "%RISK_FILTER_TEST%"
if errorlevel 1 (
  echo [ERROR] risk filter regression tests failed.
  exit /b 43
)
echo [INFO] Risk filter regression tests passed.
exit /b 0

:open_trval
if "!VERIFY_OPEN_TRVAL!"=="1" (
  if exist "!TRVAL_HTML!" (
    echo [INFO] Opening trading stage report: !TRVAL_HTML!
    start "" "!TRVAL_HTML!"
  ) else (
    echo [WARN] trading stage report not found: !TRVAL_HTML!
  )
)
