...
set "REPORT_PY=%ROOT%_runtime\python312-embed\python.exe"
if not exist "%REPORT_PY%" set "REPORT_PY=E:\1_Data\_runtime\python312-embed\python.exe"
if not exist "%REPORT_PY%" (
  echo [BTREAL][ERR] official python missing for report refresh: %REPORT_PY%
  exit /b 60
)
if not exist "%ROOT%report_backtest_v41_1.py" (
  echo [BTREAL][ERR] report_backtest_v41_1.py missing
  exit /b 60
)
"%REPORT_PY%" "%ROOT%report_backtest_v41_1.py"
if errorlevel 1 (
  echo [BTREAL][ERR] report_backtest_v41_1.py failed
  exit /b %ERRORLEVEL%
)
call :require_fresh "%ROOT%12_Risk_Controlled\report_backtest_summary_v41_1.json" "%ROOT%report_backtest_v41_1.py" report_v41_summary
if errorlevel 1 exit /b %ERRORLEVEL%
call :require_fresh "%ROOT%12_Risk_Controlled\report_backtest_trades_v41_1.csv" "%ROOT%report_backtest_v41_1.py" report_v41_trades
if errorlevel 1 exit /b %ERRORLEVEL%
...
call "%ROOT%run_backtest_validation_framework.bat" --market-csv "%ROOT%2_Logs\backtest_market_ohlc_latest.csv" --date-col date --enable-cpcv --auto-fetch-inflation --inflation-country-code KR --max-tolerable-mdd 0.30 --strategy-spec "%ROOT%tools\backtest_real_strategy_adapter.py:real_strategy_signal" --backtest-spec "%ROOT%tools\backtest_real_strategy_adapter.py:real_strategy_backtest" --params-json "{\"fast\":8,\"slow\":100,\"allow_short\":true,\"position_scale\":0.7,\"daily_gross_turnover_cap_pct\":0.008}" --grid-spec-json "{\"daily_gross_turnover_cap_pct\":[0.006,0.008,0.01,0.012]}" --cost-model-json "{\"commission_bps\":5.0,\"slippage_bps\":5.0,\"spread_bps\":5.0}" --out-json "%ROOT%2_Logs\backtest_validation_full_market_latest.json" --out-csv "%ROOT%2_Logs\backtest_validation_full_market_gates_latest.csv"
if errorlevel 1 (
  echo [BTREAL][WARN] full-market validation reported fail
)
if not exist "%ROOT%2_Logs\backtest_validation_full_market_latest.json" (
  echo [BTREAL][WARN] full-market validation report missing
)
