...
if /I "%SESSION%"=="afterhours" (
  if not defined NEWS_COLLECT_MAX_RUNTIME_SEC set "NEWS_COLLECT_MAX_RUNTIME_SEC=600"
)
if "%NEWS_OBSERVE_MAX_SYMBOLS%"=="" set "NEWS_OBSERVE_MAX_SYMBOLS=10"
if "%NEWS_GOOGLE_RSS_MAX_ITEMS%"=="" set "NEWS_GOOGLE_RSS_MAX_ITEMS=5"
if "%NEWS_CANDIDATES_TIMEOUT_SEC%"=="" set "NEWS_CANDIDATES_TIMEOUT_SEC=120"
if "%NEWS_GLOBAL_MACRO_TIMEOUT_SEC%"=="" set "NEWS_GLOBAL_MACRO_TIMEOUT_SEC=30"
if "%NEWS_GLOBAL_RISK_TIMEOUT_SEC%"=="" set "NEWS_GLOBAL_RISK_TIMEOUT_SEC=45"
if "%NEWS_GLOBAL_MACRO_MAX_ITEMS%"=="" set "NEWS_GLOBAL_MACRO_MAX_ITEMS=8"
if "%NEWS_GLOBAL_RISK_MAX_ARTICLES%"=="" set "NEWS_GLOBAL_RISK_MAX_ARTICLES=12"
set "LOCK_PARENT=%ROOT%2_Logs\locks"
set "LOCK_DIR=%LOCK_PARENT%\news_pipeline_once.lock"
if "%NEWS_PIPELINE_LOCK_STALE_MINUTES%"=="" set "NEWS_PIPELINE_LOCK_STALE_MINUTES=180"
if not exist "%LOCK_PARENT%" mkdir "%LOCK_PARENT%" >nul 2>nul
if exist "%LOCK_DIR%" (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='SilentlyContinue'; $lock=$env:LOCK_DIR; $owner=Join-Path $lock 'owner.txt'; $max=[double]$env:NEWS_PIPELINE_LOCK_STALE_MINUTES; $stamp=(Get-Item -LiteralPath $owner -ErrorAction SilentlyContinue).LastWriteTime; if (-not $stamp) { $stamp=(Get-Item -LiteralPath $lock).LastWriteTime }; $age=((Get-Date)-$stamp).TotalMinutes; $cutoff=$stamp.AddMinutes(5); $active=Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match 'run_news_pipeline_once(\\.bat|_hidden\\.vbs)' -and $_.CreationDate -le $cutoff }; if ($age -ge $max -and -not $active) { Remove-Item -LiteralPath $lock -Recurse -Force; Write-Output ('[NEWS_PIPELINE] stale lock cleared age_min={0:N1}' -f $age); exit 0 }; exit 1"
)
...
if not "%NEWS_GLOBAL_MACRO_SKIP%"=="1" (
  call :RUN_PY tools\run_step_with_timeout.py --name news_collect_global_macro_rss --timeout-sec %NEWS_GLOBAL_MACRO_TIMEOUT_SEC% --status-json 2_Logs\news_collect_global_macro_timeout_status_latest.json -- "%PY%" tools\news_collect_global_macro_rss.py --write-db --max-items-per-source %NEWS_GLOBAL_MACRO_MAX_ITEMS%
  if errorlevel 1 (
    echo [WARN] news_collect_global_macro_rss.py failed or timed out; continuing with existing news scores
  )
  call :RUN_PY tools\run_step_with_timeout.py --name news_llm_global_risk_daily --timeout-sec %NEWS_GLOBAL_RISK_TIMEOUT_SEC% --status-json 2_Logs\news_llm_global_risk_timeout_status_latest.json -- "%PY%" tools\news_llm_global_risk_daily.py --max-articles %NEWS_GLOBAL_RISK_MAX_ARTICLES%
  if errorlevel 1 (
    echo [WARN] news_llm_global_risk_daily.py failed or timed out; continuing with existing news scores
  )
)
if "%NEWS_GLOBAL_MACRO_SKIP%"=="1" (
  echo [NEWS_PIPELINE] NEWS_GLOBAL_MACRO_SKIP=1; skip global macro RSS/risk shadow layer
)
...
call :RUN_PY tools\build_publisher_trust_historical_tp_fp.py
if errorlevel 1 (
  echo [FAILED] build_publisher_trust_historical_tp_fp.py
  call :RELEASE_LOCK
  exit /b 1
)
call :RUN_PY tools\verify_publisher_trust_historical_fresh.py
if errorlevel 1 (
  echo [FAILED] verify_publisher_trust_historical_fresh.py
  call :RELEASE_LOCK
  exit /b 1
)
if not "%NEWS_MANUAL_IMPLICATION_SKIP%"=="1" (
  call :RUN_PY tools\manual_news_implications_daily.py
  if errorlevel 1 (
    echo [WARN] manual_news_implications_daily.py failed; continuing without manual overlay
  )
)
call :RUN_PY tools\news_score_daily.py
if errorlevel 1 (
...
if not "%NEWS_READONLY_ANALYSIS_SKIP%"=="1" (
  call :RUN_PY tools\extract_news_article_content_trafilatura.py
  if errorlevel 1 (
    echo [WARN] extract_news_article_content_trafilatura.py failed; continuing without content sidecar
  )
  call :RUN_PY tools\auto_news_implications_daily.py
  if errorlevel 1 (
    echo [WARN] auto_news_implications_daily.py failed; continuing without auto implication analysis
  )
  call :RUN_PY tools\build_news_topic_judgment_layer.py
  if errorlevel 1 (
    echo [WARN] build_news_topic_judgment_layer.py failed; continuing without topic judgment layer
  )
  call :RUN_PY tools\build_news_topic_candidate_impact.py
  if errorlevel 1 (
    echo [WARN] build_news_topic_candidate_impact.py failed; continuing without topic candidate impact
  )
  if not "%NEWS_L3_LAYER_SKIP%"=="1" (
    call :RUN_PY tools\build_news_judgment_l3_layer.py
    if errorlevel 1 (
      echo [WARN] build_news_judgment_l3_layer.py failed; continuing without L3 execution judgment
    )
  )
  if not "%NEWS_MEDIUM_ADJ_SKIP%"=="1" (
    call :RUN_PY tools\build_news_medium_adjustment_daily.py
    if errorlevel 1 (
      echo [WARN] build_news_medium_adjustment_daily.py failed; continuing without medium adjustment
    )
  )

  call :RUN_PY tools\run_step_with_timeout.py --name macro_probe --timeout-sec 60 --status-json 2_Logs\macro_probe_timeout.json -- "%PY%" tools\news_google_rss_probe.py --macro-mode
  call :RUN_PY tools\run_step_with_timeout.py --name macro_feed --timeout-sec 60 --status-json 2_Logs\macro_feed_timeout.json -- "%PY%" tools\build_global_event_feed_from_google_rss.py
  call :RUN_PY tools\run_step_with_timeout.py --name macro_shadow --timeout-sec 60 --status-json 2_Logs\macro_shadow_timeout.json -- "%PY%" tools\news_llm_global_shadow_daily.py
  call :RUN_PY tools\run_step_with_timeout.py --name domestic_shadow --timeout-sec 60 --status-json 2_Logs\domestic_shadow_timeout.json -- "%PY%" tools\news_llm_domestic_shadow_daily.py
)
if "%NEWS_READONLY_ANALYSIS_SKIP%"=="1" (
  echo [NEWS_PIPELINE] NEWS_READONLY_ANALYSIS_SKIP=1; skip readonly analysis layers
)
...
if not "%NEWS_PIPELINE_SKIP_FINAL%"=="1" (
  if not "%NEWS_CANDIDATES_SKIP%"=="1" (
    call :RUN_PY tools\run_step_with_timeout.py --name news_candidates_daily --timeout-sec %NEWS_CANDIDATES_TIMEOUT_SEC% --status-json 2_Logs\news_candidates_timeout_status_latest.json -- "%PY%" tools\news_candidates_daily.py
    if errorlevel 1 (
  ...
  if errorlevel 1 (
    echo [FAILED] build_news_signal_shadow_stage.py
    call :RELEASE_LOCK
    exit /b 1
  )
  call :RUN_PY tools\build_news_signal_shadow_diagnostic.py
  if errorlevel 1 (
    echo [FAILED] build_news_signal_shadow_diagnostic.py
    call :RELEASE_LOCK
    exit /b 1
  )
  call :RUN_PY tools\build_news_signal_market_corroboration_audit.py
  if errorlevel 1 (
    echo [FAILED] build_news_signal_market_corroboration_audit.py
    call :RELEASE_LOCK
    exit /b 1
  )
)
