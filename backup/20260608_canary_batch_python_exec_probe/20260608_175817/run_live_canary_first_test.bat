...
if not exist "%PY%" if exist "%ROOT%_runtime\python312-embed\python.exe" set "PY=%ROOT%_runtime\python312-embed\python.exe"
if not exist "%PY%" if exist "%ROOT%.venv\Scripts\python.exe" set "PY=%ROOT%.venv\Scripts\python.exe"
if not exist "%PY%" if exist "E:\vibe\buffett\.venv\Scripts\python.exe" set "PY=E:\vibe\buffett\.venv\Scripts\python.exe"
if not exist "%PY%" (
...
call "%PY%" tools\kis_live_canary_first_test.py %ARGS%
set "RC=%ERRORLEVEL%"
