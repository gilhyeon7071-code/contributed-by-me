...
call "%ROOT%run_backtest_validation_framework.bat" --market-csv "%ROOT%2_Logs\backtest_market_ohlc_latest.csv" --date-col date --enable-cpcv --auto-fetch-inflation --inflation-country-code KR --max-tolerable-mdd 0.30 --strategy-spec "%ROOT%tools\backtest_real_strategy_adapter.py:real_strategy_signal" --backtest-spec "%ROOT%tools\backtest_real_strategy_adapter.py:real_strategy_backtest" --params-json "{\"fast\":8,\"slow\":100,\"allow_short\":true,\"position_scale\":0.7,\"daily_gross_turnover_cap_pct\":0.01}" --grid-spec-json "{\"daily_gross_turnover_cap_pct\":[0.008,0.01,0.012]}" --cost-model-json "{\"commission_bps\":5.0,\"slippage_bps\":5.0,\"spread_bps\":5.0}" --out-json "%ROOT%2_Logs\backtest_validation_full_market_latest.json" --out-csv "%ROOT%2_Logs\backtest_validation_full_market_gates_latest.csv"
if errorlevel 1 (
