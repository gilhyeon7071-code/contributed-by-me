...
powershell -NoProfile -Command "$market = '%ROOT%2_Logs\backtest_market_ohlc_latest.csv'; $ledger = '%LEDGER_CSV%'; $out = '%REAL_MARKET_CSV%'; if (!(Test-Path -LiteralPath $market)) { exit 71 }; if (!(Test-Path -LiteralPath $ledger)) { exit 72 }; $ledgerRows = Import-Csv -LiteralPath $ledger; $minDay = ($ledgerRows | ForEach-Object { $_.date } | Where-Object { $_ } | Sort-Object | Select-Object -First 1); if (-not $minDay) { exit 73 }; $minDate = [datetime]::ParseExact($minDay, 'yyyyMMdd', $null); $rows = Import-Csv -LiteralPath $market | Where-Object { $_.date -and ([datetime]$_.date) -ge $minDate }; if (($rows | Measure-Object).Count -eq 0) { exit 74 }; $rows | Export-Csv -LiteralPath $out -NoTypeInformation -Encoding UTF8"
if errorlevel 1 (
