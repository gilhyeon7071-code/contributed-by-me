...
for root in roots:
    for dp,_,fns in os.walk(root):
        p=dp.replace('/','\\').lower()
        if any(k in p for k in ["\\backup\\","\\_backup\\","\\_tmp\\","\\2_logs\\","\\_notes\\","\\tools_archive\\","\\.git\\","\\__pycache__\\","\\_bulk_backup_"]):
            continue
        for fn in fns:
            low=fn.lower()
            ...
            if any(k in lf for k in ["\\_notes\\","\\2_logs\\","\\_bulk_backup_"]):
                continue
            files.append(full)
for p in files:
    try:
...
out=r"E:\\1_Data\\_tmp_syntax_errors_operational_final.json"
